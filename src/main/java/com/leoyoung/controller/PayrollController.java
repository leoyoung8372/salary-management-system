...
public class PayrollController {
    ...
    // 生成PDF文件
    @GetMapping("/pdf")
    public ResponseEntity<?> getPayrollPdf(@RequestParam("employeeId") String employeeId,
                                           @RequestParam("salaryDate") String salaryDate) {

        // 根据员工ID和工资日期获取工资单信息
        Payroll payroll = payrollService.getPayrollByEmployeeIdAndDate(employeeId, salaryDate);

        if (payroll == null) {
            // 如果工资记录不存在，返回404 Not Found和错误信息
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("不存在该薪资记录。");
        }
